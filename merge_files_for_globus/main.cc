...
void splitline(const std::string &s, std::vector<std::string> *vec,
  ...
  if (!vec) throw std::domain_error("splitline: called on null vector pointer");
  vec->clear();
...
class annotation {
 public:
  ...
  void add_data(const std::string &tag, unsigned pos, const std::string &id,
                const std::string &ref, const std::string &alt,
                const std::string &freq, const double &beta, const double &se,
                const double &p, unsigned n, const double &phet) {
    ...
    if (_ref.empty() && _alt.empty()) {
      ...
      if (freq.find_first_not_of("1234567890.") != std::string::npos) {
        ...
        _freqs.push_back(std::to_string(1.0 - from_string<double>(freq)));
      }
  ...
  void report_entry(const std::string &tag, bool first, bool report_frequency,
                    std::ostringstream &o) {
    ...
    if ((finder = _tags.find(tag)) == _tags.end()) {
      ...
      o << '\t' << _ps.at(finder->second) << '\t' << _ns.at(finder->second);
...
void read_data(
  ...
  try {
    ...
    while (input->getline(&line)) {
      ...
      finder->second->add_data(file_tag, pos, id, ref, alt, freq, beta, se, p,
                               n, hetp);
    }
    input->close();
    delete input;
...
void write_data(
  ...
  try {
    ...
    for (std::map<std::string,
             ...
             file_linker.begin();
         iter1 != file_linker.end(); ++iter1) {
      header << "\tFREQ_" << iter1->first;
      for (std::map<std::string, std::string>::const_iterator iter2 =
               iter1->second.begin();
           iter2 != iter1->second.end(); ++iter2) {
        header << "\tBETA_" << iter1->first << "_" << iter2->first << "\tSE_"
               << iter1->first << "_" << iter2->first << "\tP_" << iter1->first
               << "_" << iter2->first << "\tN_" << iter1->first << "_"
               << iter2->first << "\tPHet_" << iter1->first << "_"
               << iter2->first;
      }
    }
    output->writeline(header.str());
    for (unsigned i = 0; i < target.size(); ++i) {
      for (std::map<std::string, boost::shared_ptr<annotation> >::const_iterator
               iter = target.at(i).begin();
           iter != target.at(i).end(); ++iter) {
        std::ostringstream o;
        o << (i + 1);
        for (std::map<std::string,
                      std::map<std::string, std::string> >::const_iterator
                 iter1 = file_linker.begin();
             iter1 != file_linker.end(); ++iter1) {
          for (std::map<std::string, std::string>::const_iterator iter2 =
                   iter1->second.begin();
               iter2 != iter1->second.end(); ++iter2) {
            iter->second->report_entry(
                iter1->first + "_" + iter2->first,
                iter1 == file_linker.begin() && iter2 == iter1->second.begin(),
                iter2 == iter1->second.begin(), o);
          }
        }
...
int main(int argc, char **argv) {
  if (argc < 4) {
    throw std::domain_error("usage: \"" + std::string(argv[0]) +
                            " [assorted analysis files] output_filename [yes "
                            "sex-specific or no]\"");
  }
  ...
  for (std::map<std::string,
    ...
    for (std::map<std::string, std::string>::const_iterator iter2 =
      ...
      if (!iter2->second.empty()) {
        std::cout << "reading data from file \"" << iter2->second << "\""
                  << std::endl;
