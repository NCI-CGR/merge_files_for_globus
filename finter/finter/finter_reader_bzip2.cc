...
merge_files_for_globus::finter_reader_bzip2::finter_reader_bzip2()
    : finter_reader(),
      _raw_input(0),
      _bz_input(0),
      _eof(false),
...
void merge_files_for_globus::finter_reader_bzip2::open(const char *filename) {
  if (_raw_input)
    throw std::domain_error(
        "merge_files_for_globus::finter_reader_bzip2: attempted to reopen "
        "in-use handle");
  _raw_input = fopen(filename, "r");
  if (!_raw_input)
    throw std::domain_error(
        "merge_files_for_globus::finter_reader_bzip2: cannot open file \"" +
        std::string(filename) + "\"");
  int error = 0;
  _bz_input = BZ2_bzReadOpen(&error, _raw_input, 0, 0, NULL, 0);
  if (error == BZ_CONFIG_ERROR) {
    throw std::domain_error(
        "merge_files_for_globus::finter_reader_bzip2::open: bzip2 reading "
...
void merge_files_for_globus::finter_reader_bzip2::close() {
  if (_bz_input) {
    int error = 0;
    BZ2_bzReadClose(&error, _bz_input);
    if (error == BZ_SEQUENCE_ERROR)
      throw std::domain_error(
          "merge_files_for_globus::finter_reader_bzip2::close: bzip2 reports "
...
void merge_files_for_globus::finter_reader_bzip2::clear() {
  _good = true;
  _bad = _fail = false;
  _buf_remaining = _buf_read = 0;
}

bool merge_files_for_globus::finter_reader_bzip2::is_open() const {
  return (_raw_input && _bz_input);
}

char merge_files_for_globus::finter_reader_bzip2::get() {
  if (!_buf_remaining) {
    refresh_buffer();
  }
  if (!_buf_remaining)
    throw std::domain_error(
        "merge_files_for_globus::finter_reader_bzip2::get: end of file");
  --_buf_remaining;
  return _buf[_buf_read - (_buf_remaining + 1)];
}

bool merge_files_for_globus::finter_reader_bzip2::getline(std::string *res) {
  if (!res)
...
bool merge_files_for_globus::finter_reader_bzip2::eof() const { return _eof; }

bool merge_files_for_globus::finter_reader_bzip2::good() const {
  return _good;
}

bool merge_files_for_globus::finter_reader_bzip2::bad() const { return _bad; }

void merge_files_for_globus::finter_reader_bzip2::read(char *target,
                                                         std::streamsize n) {
  ...
  if (amount_read < n && !eof()) {
    ...
    if (error == BZ_PARAM_ERROR || error == BZ_IO_ERROR ||
      ...
      throw std::domain_error(
          "merge_files_for_globus::finter_reader_bzip2::read: bzip2 reports "
...
void merge_files_for_globus::finter_reader_bzip2::refresh_buffer() {
  if (!eof()) {
    int error = 0;
    int num_read =
    ...
    if (error == BZ_PARAM_ERROR || error == BZ_IO_ERROR ||
      ...
      throw std::domain_error(
          "merge_files_for_globus::finter_reader_bzip2::refresh_buffer: "
          "bzip2 reports "
          "read called on stream opened as write");
    }
