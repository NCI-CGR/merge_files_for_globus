...
void merge_files_for_globus::finter_reader_flat::open(const char *filename) {
  _input.open(filename);
  if (!_input.is_open())
    throw std::domain_error(
        "merge_files_for_globus::finter_reader_flat::open: cannot open file "
        "\"" +
        std::string(filename) + "\"");
}

bool merge_files_for_globus::finter_reader_flat::getline(std::string *line) {
  if (!line)
...
void merge_files_for_globus::finter_reader_flat::read(char *buf,
                                                        std::streamsize n) {
  _input.read(buf, n);
}
